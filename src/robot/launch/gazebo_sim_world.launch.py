...
def generate_launch_description():
    ...
    bridge_config_file = os.path.join(this_pkg, 'config', 'ros_gz_bridge.yaml')
    tf_bridge_config_file = os.path.join(this_pkg, 'config', 'ros_gz_tf_bridge.yaml')
    ...
    bridge_tf = DeclareLaunchArgument(
        ...
        default_value='true',
        description=(
            'Bridge Gazebo model TF. Disable when FAST-LIO publishes '
            'map->base_footprint.'
        ),
    )
    ...
    gazebo_tf_bridge = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_ros_gz_bridge, 'launch', 'ros_gz_bridge.launch.py')
        ),
        launch_arguments={
            'bridge_name': 'ros_gz_tf_bridge',
            'config_file': tf_bridge_config_file,
            'use_composition': 'False',
        }.items(),
